import CreditsProfitHistoriesDataTableSection from "@/components/(common)/credits-profits-details-page/CreditsProfitHistoriesDataTableSection";
import CreditsProfitHistoryViewModal from "@/components/modals/CreditsProfitHistoryViewModal";
...
const CreditsProfitsDetailsPage = () => {
  const { id } = useParams();
  const location = useLocation();
  const dispatch = useDispatch();

  const creditsProfit = (location.state as { creditsProfit?: TCreditsProfit })
    ?.creditsProfit;

  const { isHistoryModalOpen, selectedCreditsProfit } = useSelector(
    (state: RootState) => state.creditsProfitsPage,
  );

  const onOpenHistoryModal = () => {
    dispatch(openHistoryModal(selectedCreditsProfit || ({} as any)));
  };
  ...
  const {
    ...
    queryKey: ["credits-profit", id],
    queryFn: () => fetchCreditsProfit(id || ""),
    enabled: !!id,
  });

  const { data: historiesData, isLoading: historiesLoading } = useQuery({
    queryKey: ["credits-profit-histories", id],
    queryFn: () => fetchCreditsProfitHistories(id || ""),
    enabled: !!id,
  });

  const currentProfit = profitResponse?.data || creditsProfit;
  ...
  return (
    <div className="bg-background p-4">
      <div className="container space-y-6">
        {/* Header */}
        <PageHeader
          name="Credits Profit Details"
          description="View credits profit information and history"
          slot={<TrendingUp />}
        />
        ...
        <Card>
          ...
          <Card.Content>
            <CreditsProfitHistoriesDataTableSection
        ...
        {/* Modals */}
        <CreditsProfitHistoryViewModal
          creditsProfitId={id || ""}
          isOpen={isHistoryModalOpen}
          setIsOpen={(value: boolean) =>
            dispatch(
              value
                ? openHistoryModal(selectedCreditsProfit || ({} as any))
                : closeHistoryModal(),
            )
          }
        />
...
export default CreditsProfitsDetailsPage;
