...
import { createFeaturePopup } from "@/services/feature-popup.service";
...
type FeaturePopupAddModalProps = {
  default?: Partial<TFeaturePopup>;
  isOpen: boolean;
...
const FeaturePopupAddModal: React.FC<FeaturePopupAddModalProps> = ({
  isOpen,
  setIsOpen,
  default: popup,
  ...
  const mutation = useMutation({
    mutationFn: createFeaturePopup,
    onSuccess: (data) => {
      toast.success(data?.message || "Feature popup created successfully!");
  ...
  const onSubmit = (
    ...
    // Exclude image and video from TFeaturePopup (which are strings) and use File versions
    ...
    <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
      <Modal.Backdrop>
        <Modal.Content className="max-h-[90vh] max-w-4xl overflow-y-auto">
          <Modal.Header>
            <Modal.Title>Add Feature Popup</Modal.Title>
            <Modal.Close />
          </Modal.Header>
...
export default FeaturePopupAddModal;
